...
def prepare_sim_train_data(generated_file_path, rpts):
    ...
    result_df = result_df.reindex(
        columns=[PHENOTYPE_ID_COL_NAME, LABEL_COL_NAME] + [tool for tool, _, _, _ in TOOL_SIG_COL_INFO], copy=False)
    return result_df
...
def train_model_xgb(input_df, test_result_file):
    ...
    if input_df is None or input_df.shape[0] == 0:
        ...
        return
    x_df = input_df[[tool for tool, _, _, _ in TOOL_SIG_COL_INFO]]
...
def prepare_ranking_input(rpts):
    ...
    prediction_required_cols = [PHENOTYPE_ID_COL_NAME] + [tool for tool, _, _,_ in TOOL_SIG_COL_INFO]
...
def run_ranking(rpt=None, output_file_path=None):
    ...
    x_df = target_df[[tool for tool, _, _,_ in TOOL_SIG_COL_INFO]]
    model_path = os.path.join(os.path.dirname(Path(__file__).resolve()), 'model', 'ensemble.ubj')
    if Path(model_path).exists():
        model = xgb.XGBRegressor()
        model.load_model(model_path)
...
if __name__ == '__main__':
    ...
    rpt_set = []
    all_tool_list = [tool for tool, _, _,_ in TOOL_SIG_COL_INFO]
