...
def prepare_rra_input(output_file_path, rpts, qtl_type):
    ...
    for tool, sig_column, sig_type, lead_variant in TOOL_SIG_COL_INFO:
        ...
        if tool_rpt is None or len(tool_rpt) == 0:
            continue
        df_list.append(read_tool_result(tool_rpt, tool_name=tool, sig_col_name=sig_column, sig_type=sig_type, qtl_type=qtl_type))
...
def read_tool_result(rpt, tool_name, sig_col_name, sig_type, qtl_type):
    ...
    rpt_df = rpt_df[[sig_col_name, PHENOTYPE_ID_COL_NAME]]
    ...
    rpt_df[tool_name] = rpt_df[PHENOTYPE_ID_COL_NAME]
    rpt_df['qtl_type'] = qtl_type
    rpt_df.drop(labels=[PHENOTYPE_ID_COL_NAME], axis=1, inplace=True)
    rpt_df[RANK_COL_NAME] = range(1, rpt_df.shape[0] + 1)
    return rpt_df


def run_ranking(output_file_path, rpt=None, sample_size=None, method=None, qtl_type=None):
    
    
    ranking_input_file = os.path.join(os.path.dirname(output_file_path), f'pre_{output_file_path.split(os.sep)[-1]}')
    prepare_rra_input(output_file_path=ranking_input_file, rpts=rpt, qtl_type=qtl_type)
