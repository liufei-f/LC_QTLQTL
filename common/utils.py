...
def split_file_by_col_name_qtl(working_dir, src_file_path, chrom_col, pheno_col, readonly_cols=None, dtype=None,
    ...
    #         f'Input file {src_file_path} does not exist or size is 0, nothing to split')
    Path(working_dir).mkdir(parents=True, exist_ok=True)
    filelist = glob(src_file_path)
    if len(filelist) == 0:
        warnings.warn(
            f'Input file {src_file_path} does not exist or size is 0, nothing to split')
    for file in filelist:
        logging.info(f"QTL file: {file}")
        with pd.read_table(file, sep=sep, iterator=True, chunksize=100000, header=0,
                        usecols=readonly_cols, dtype=dtype) as reader:
            for chunk in reader:
                grouped = chunk.groupby(pheno_col)
                for name, group in grouped:
...
def get_eqtl_gene_name(file_path):
    ...
    if len(file_path.split(os.sep)[-1].split('.')) == 3:
        ...
        logging.info(f"utils_eqtl_gene_name: {name}")
        return name
    else:
        name = '.'.join(file_path.split(os.sep)[-1].split('.')[:-2])
        logging.info(f"utils_eqtl_gene_name: {name}")
        return name


def get_sqtl_pheno_name(file_path):
    # return file name without extension
    logging.info(f"utils_sqtl_file_path: {file_path}")
    name = '.'.join(file_path.split(os.sep)[-1].split('.')[:-2])
    logging.info(f"utils_sqtl_pheno_name: {name}")
    return name

def get_caqtl_pheno_name(file_path):
    # return file name without extension
    logging.info(f"utils_caqtl_file_path: {file_path}")
    name = '.'.join(file_path.split(os.sep)[-1].split('.')[:-2])
    logging.info(f"utils_caqtl_pheno_name: {name}")
    return name

def get_qtl_pheno_name(file_path):
    # return file name without extension
    logging.info(f"utils_qtl_file_path: {os.path.basename(file_path)}")
    name = file_path.strip(".tsv.gz")
    logging.info(f"utils_qtl_pheno_name: {name}")
    return name
...
def delete_dir(file_path, keep_intermediate=False):
...
def extract_vcf_data(chromosome, target_pos_rsid_df, ref_vcf_file_path, output_vcf_dir, output_vcf_file_name,
                     target_position_col_name, target_snp_col_name, extract_step_size=10000):
    logging.info("Start extract_vcf")
    ...
    logging.info(f"chrom_code_in_vcf: {chrom_code_in_vcf}")
    ...
    print(f"output_file_full_path {output_file_full_path}")
    print(f"tmp_vcf_file_full_path {tmp_vcf_file_full_path}")
    print(f"matching_snp_dir {matching_snp_dir}")
    print(f"report_file_name {report_file_name}")
    print(f"output_matching_snp_report {output_matching_snp_report}")
    print(f"output_missing_snp_report {output_missing_snp_report}")

    # writing header
    print(f"shell_command_write_vcf_header")
    print(shell_command_write_vcf_header.format(ref_vcf_file_path, tmp_vcf_file_full_path))
    ...
    with open(tmp_vcf_file_full_path, 'a') as output_file_obj:
        while start_pos < max_pos + 1:
            logging.info(f"start_pos < max_pos: {start_pos} < {max_pos}")
            ...
            for line in iter(process.stdout.readline, b''):
                ...
                # logging.info(f"snp_pos: {snp_pos}")
    ...
    print(pos_df)
    if pos_df.empty:
        print(f'**** end process, pos_df is empty')
        ...
        logging.info(f'Extract_vcf_data: **** end process, output file is {output_file_full_path}')
...
def extract_r2_from_ld_file(file_path):
    with open(file_path, 'r') as f:
        for line in f:
            if "R-sq =" in line:
                parts = line.strip().split()
                r2_index = parts.index("=") + 1
                r2_value = float(parts[r2_index])
                return r2_value
    return None  # 如果没找到 R-sq
...
def parse_parameters():
    ...
    parser.add_argument('--keep_intermediate', dest='keep_intermediate', help='keep intermediate files', action="store_true")
...
def get_predixcan_ref_files(global_config):
    ...
    eqtl_issue_name = global_config['input']['qtl1']['biological_context']
    model_db_path = None
...
def get_ref_ldblock_vcf_path(population, chrom, variant_position, ref_ldblock_vcf_dir):

    '''
    e.g.
    chrom: 1
    variant_position: 1234567
    population: EUR
    ref_ldblock_vcf_dir: /path/to/ldblock/vcfs
    return: /path/to/ldblock/vcfs/EUR/1_1230000_1240000.vcf.gz
    '''
    ld_block_bed_dir = os.path.join(os.path.dirname(Path(__file__).parent.resolve()), 'reference', 'LDblocks')
    ld_block_bed_path = os.path.join(ld_block_bed_dir, f'{population.upper()}_LD_blocks.bed')
    print(f"DEBUG: ld_block_bed_dir {population} {chrom} {variant_position} : {ld_block_bed_path}")
    ld_block_bed_file = glob(ld_block_bed_path)[0]
    ld_block = pd.read_csv(ld_block_bed_file, sep='\t')
    ld_block = ld_block[(ld_block['chr'] == f'chr{chrom}') &
                        (ld_block['start'] <= variant_position) &
                        (ld_block['end'] >= variant_position)]
    if ld_block.empty:
        return None
    print(f"DEBUG: {population} {chrom} {variant_position} ld_block: {ld_block}")
    file_name = f'{chrom}_{ld_block.iloc[0]["start"]}_{ld_block.iloc[0]["end"]}_ldblock.vcf.gz'
    file_path = os.path.join(ref_ldblock_vcf_dir, population.upper(), file_name)
    print(f"DEBUG: {file_path}")
    return file_path


def get_ref_ldblock_ld_path(population, chrom, variant_position, ref_ldblock_vcf_dir):
    ...
    population: EUR
    ref_ldblock_vcf_dir: /path/to/ldblock/vcfs
    return: /path/to/ldblock/vcfs/EUR/1_1230000_1240000.ld.gz
    '''
    ld_block_bed_dir = os.path.join(os.path.dirname(Path(__file__).parent.resolve()), 'reference', 'LDblocks')
    ld_block_bed_file = glob(os.path.join(ld_block_bed_dir, f'{population.upper()}_LD_blocks.bed'))[0]
    ld_block = pd.read_csv(ld_block_bed_file, sep='\t')
    ld_block = ld_block[(ld_block['chr'] == f'chr{chrom}') &
                        (ld_block['start'] <= variant_position) &
                        (ld_block['end'] >= variant_position)]
    if ld_block.empty:
        return None
    file_name = f'{chrom}_{ld_block.iloc[0]["start"]}_{ld_block.iloc[0]["end"]}_ldblock.ld.gz'
    file_path = os.path.join(ref_ldblock_vcf_dir, population.upper(), file_name)
    print(f"DEBUG: {file_path}")
    return file_path
